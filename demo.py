...
class ASDDemo(object):
    ...
    def load_model(self):
        ...
        try:
            self.modelzoos['asd'] = mlib.OnePassASD_MultiHeads(self.args)
            if self.device == 'cuda':
            ...
            if os.path.isfile(self.args.pretrain):
                self.modelzoos['asd'] = ulib.load_weights(
                    ...
                    device=self.device)
                print('loading checkpoints from %s ...' % self.args.pretrain.split('/')[-1])
            else:
    ...
    def detect_video_faces(self):
        ...
        for frame in tqdm.tqdm(self.data['frames']):
            ...
            try:
                ...
                det_bboxes = self.modelzoos['detector'].detect_faces(
                    frame, self.args.conf_thresh, self.args.scalelist)
    ...
    def visualize_track_segs(self):
        if not self.args.is_debug:
            return 
        print('step09. [optional] visualize middle trackseg ...')
        video_fps = 25
            ...
            for dur in self.args.duration_list:
                ...
                with torch.set_grad_enabled(False):
                    for i in range(num_dur_units):
                        ...
                        inp_audio = torch.FloatTensor(inp_audio).unsqueeze(0).cuda()
                        inp_faces = faces[i * dur * 25 : (i + 1) * dur * 25, :, :]
                        inp_faces = torch.FloatTensor(inp_faces).unsqueeze(0).cuda()
    ...
    def visualize_demo(self):
        ...
        for vf_idx, frame in enumerate(self.data['frames']):
            ...
            videoio.write(frame)
        videoio.release()
        demo_file = os.path.join(self.args.test_dir, 'demos', '%s_4.avi' % base_name)
        cmd = 'ffmpeg -y -i %s -i %s -threads %d -c:v copy -c:a copy %s -loglevel quiet' % (\
    ...
    def run_demo(self, test_video='', is_25_fps=False):
        ...
        # middle_file = 'testdemo/middle/demo.npy'
