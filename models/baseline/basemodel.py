...
from models.baseline.loss import lossAV, lossA, lossV
from models.baseline.audioEncoder  import audioEncoder
from models.baseline.visualEncoder import visualFrontend, visualTCN, visualConv1D
from models.baseline.attentionLayer import attentionLayer
...
class talkNet(nn.Module):
    ...
    def forward(self, a, v):
        a = self.model.forward_audio_frontend(a)
        v = self.model.forward_visual_frontend(v)
        a, v = self.model.forward_cross_attention(a, v)
        f = self.model.forward_audio_visual_backend(a, v)
        f = self.lossAV.forward(f)
        return (f, None, None)
