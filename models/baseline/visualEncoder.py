...
class visualFrontend(nn.Module):
    ...
    def forward(self, x):
        batchsize = x.shape[0]  # [B, C, T, H, W]
        x = self.frontend3D(x)  # [B, C', T, H/4, W/4]
        x = x.transpose(1, 2)   # [B, T, C', H/4, W/4]
        x = x.reshape(-1, x.shape[2], x.shape[3], x.shape[4]) # [B*T, C', H/4, W/4]
        x = self.resnet(x)      # [B*T, C'', 1, 1] 
        x = x.reshape(batchsize, -1, 512) # [B, T, C'']
        return x
