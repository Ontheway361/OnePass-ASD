...
class CaptureCameraAV(object):
    ...
    def fetch_seq_frames(self):
        # fps-24
        print(' -step3.3. fetch avdata in parallel style ...')
    ...
    def fetch_seq_audio(self):
        # fps-16
        print(' -step3.2. fetch avdata in parallel style ...')
    ...
    def cache_buffer(self):
        num_fetch_frames = len(self.data['frame_seq']) * 2 // 3
        num_fetch_audios = len(self.data['audio_seq']) * 2 // 3
        # num_fetch_frames = -1
        # num_fetch_audios = -1
        # num_fetch_frames = 25
        # buffer_frames = self.data['frame_seq'][:num_fetch_frames]
        # buffer_audios = self.data['audio_seq'][:num_fetch_audios]

        self.data['frame_buffer'] = self.data['frame_seq'][:num_fetch_frames]
        self.data['audio_buffer'] = self.data['audio_seq'][:num_fetch_audios]
        print(len(self.data['frame_seq']), len(self.data['audio_seq']))
    ...
    def start_capture(self):
        print(' -step3.1 fetch avdata in parallel style ...')
...
if __name__ == "__main__":
    ...
    # print(len(avio.data['frame_seq']), len(avio.data['audio_seq']))
    # print(audios.shape)
    # cv2.imwrite('outcamera.png', frames[-1])
    avio.stop_capture()